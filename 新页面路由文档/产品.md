...
/**
 * 简化版演示页
 * - 移除 LeveragePicker 组件及其复杂逻辑（presets, snapping, step buttons, copy button, Radix Slider）
 * - 将杠杆输入改为简单的 HTML Range + Number Input，与本金输入保持一致。
 * - 定价系数 k 已固定为 1.00，等待后续逻辑接入。
 */
