...
const BASESCAN_TX = (hash) => `https://basescan.org/tx/${hash}`; // Base主网(8453)
const BASESCAN_ADDR = (addr) => `https://basescan.org/address/${addr}`;
...
function fmtUSDC(n) {
  ...
  const x = Number(n);
  return x.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + " USDC";
}
...
export default function TransparencyPage() {
  ...
  const [distribution, setDistribution] = useState({ buckets: [] });
  ...
  const kpis = useMemo(() => {
    ...
    return [
      ...
      { label: "7D保费", value: fmtUSDC(premiumUSDC), icon: <Wallet size={16} /> },
      { label: "7D已赔付", value: fmtUSDC(paidUSDC), icon: <ShieldCheck size={16} /> },
      ...
      { label: "金库余额", value: fmtUSDC(treasuryBalance), icon: <Database size={16} /> },
    ];
  }, [overview]);
              ...
              {showCumulative ? (
                <ResponsiveContainer>
                  <AreaChart data={series} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                    ...
                    <YAxis />
                    <Tooltip formatter={(v) => fmtUSDC(v).replace(" USDC", "")} labelFormatter={(l) => `UTC ${l}`} />
                    <Legend />
            ...
            {gauge.pct === null ? (
              ...
              <div>
                ...
                <div className="mt-2 text-sm flex justify-between">
                  <span>余额 {fmtUSDC(gauge.treasuryBalance)}</span>
                  <span>需求 {fmtUSDC(gauge.requiredReserve)}</span>
                </div>
            ...
            <div className="h-64">
              <ResponsiveContainer>
                <PieChart>
                  ...
                  <Tooltip formatter={(v, n, p) => [metric === "count" ? `${v} 份` : fmtUSDC(v).replace(" USDC", ""), p?.payload?.label]} />
                  <Legend />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div className="mt-2 text-sm text-stone-500">合计：{metric === "count" ? fmtInt(donutData.total) + " 份" : fmtUSDC(donutData.total)}</div>
          </div>
                ...
                <tbody>
                  {events.map((e, i) => (
                    <tr key={i} className="border-t">
                      ...
                      <td className="py-2 pr-4">{fmtUSDC(e.amount_usdc)}</td>
          ...
          {auditOpen && audit && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 font-mono break-all text-sm"><span>{audit.docHash || "-"}</span>{audit.docHash && <button onClick={() => copy(audit.docHash)} className="inline-flex items-center text-xs text-stone-500 hover:text-stone-700" title="复制docHash"><Copy size={14} /></button>}</div>
              </div>
              <div>
                <div className="text-sm text-stone-500 mb-1">证据根（近期）</div>
                <ul className="text-sm space-y-1">
                  {audit.evidenceRoots?.map((r, i) => (
                    <li key={i} className="font-mono break-all">{r.label ? `[${r.label}] ` : ""}{r.merkleRoot || r.root}</li>
                  ))}
                </ul>
              </div>
            </div>
          )}
          <div className="mt-3 text-xs text-stone-500">注：所有数据按UTC聚合。默认窗口=7D；历史不清空，以 docHash 公告为准。</div>
        </div>
      </div>
    </div>
  );
      ...
      {opts.map((o) => (
        <button
          ...
          className={classNames(
            ...
            value === o.k ? "bg-white shadow" : "text-stone-500 hover:bg-white/60"
          )}
        >
          {o.label}
        </button>
...
function mapEventType(t) {
  switch ((t || "").toLowerCase()) {
    ...
    case "policypurchased":
      return "PolicyPurchased";
    case "claim_paid":
    case "claimpaid":
      return "ClaimPaid";
    case "reserve_topup":
    case "reservetopup":
      return "ReserveToppedUp";
    case "reserve_withdraw":
    case "reservewithdraw":
      return "ReserveWithdrawn";
    default:
      return t || "-";
  }
}
