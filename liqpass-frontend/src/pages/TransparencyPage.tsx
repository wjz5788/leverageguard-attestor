              ...
              {showCumulative ? (
                <ResponsiveContainer>
                  <AreaChart data={series} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                    ...
                    <YAxis />
                    <Tooltip formatter={(v: number | string) => typeof v === 'number' ? fmtUSDC(v).replace(" USDC", "") : String(v)} labelFormatter={(l) => `UTC ${l}`} />
                    <Legend />
            ...
            <div className="h-64">
              <ResponsiveContainer>
                <PieChart>
                  ...
                  <Tooltip formatter={(v: number | string, n: string, p: any) => [metric === "count" ? `${v} 份` : typeof v === 'number' ? fmtUSDC(v).replace(" USDC", "") : String(v), p?.payload?.label]} />
                  <Legend />
                </PieChart>
              </ResponsiveContainer>
            </div>
