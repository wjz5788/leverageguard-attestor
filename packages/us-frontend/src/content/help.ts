...
export const tocOrder: Array<{ id: string; label: LocalizedText }> = helpSections.map((section) => ({
  id: section.id,
  label: section.title,
}));

export function getLocalizedText(text: LocalizedText, lang: SupportedLanguage): string {
  if (lang === 'en' && text.en.trim()) {
    return text.en;
  }
  return text.zh;
}
